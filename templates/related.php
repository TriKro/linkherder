...
<a class="nav-tab " href="<?= LINKHERDER_ADMIN_URL ?>">Overview</a>
	...
	<a class="nav-tab nav-tab-active" href="<?= LINKHERDER_ADMIN_URL . '&kws_action=related' ?>">Links Needed</a>
</h2>
...
<p>Last update: <?= $kws_options['last_update']? date('Y-m-d H:i', $kws_options['last_update']).", {$total_keywords} keywords" : 'Never'?> <span><input class="button" type="submit" value="Update now" onclick="window.location = '<?= LINKHERDER_ADMIN_URL ?>' + '&kws_action=update_now_related'; this.parentNode.innerHTML = 'Updating... Please wait...'" /></span></p>
<p> Your keywords will update automatically every day.
	...
	<form action="<?= LINKHERDER_ADMIN_URL ?>" method="get" style="text-align:right;">
...
<? if ($related): ?>
<form action="<?= LINKHERDER_ADMIN_URL ?>" method="post">
<div class="tablenav">
...
<span class="edit"><a target="_blank" class="thickbox" href="<?= admin_url('admin-ajax.php') ?>?action=kws_related_urls&kws_keyword=<?= (urlencode($item['keyword'])) ?>" title="Related URLs for keyword '<?= htmlspecialchars($item['keyword']) ?>'">Show related</a> |</span>
	<span class="trash"><a class="submitdelete" title="Blacklist keyword" href="<?= LINKHERDER_ADMIN_URL ?>&kws_action=related_form&related_action=blacklist&keyword[]=<?= $item['id'] ?>">Blacklist Keyword</a> |
	</span>
	<span class="trash"><a href="<?= LINKHERDER_ADMIN_URL ?>&kws_action=related_form&related_action=detach&keyword[]=<?= $item['id'] ?>" title="Detach keyword from this URL" rel="permalink">Detach from this URL</a></span>
</div>
</td>			
